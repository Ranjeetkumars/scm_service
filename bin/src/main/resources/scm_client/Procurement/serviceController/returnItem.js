...
function getSuppliers() {
	try {
		$('#vendor_id').empty();
		var strUrl = Service.LOAD_SUPPLIERS;
...
function loadBaselocations(zone_id, baselocation_id) {
	try {
		...
		$(id).empty();
		var strUrl = Service.LOAD_BASELOCATIONS;
...
function loadAmbulances(baselocation_id, ambulance_id) {
	try {
		var id = '#' + ambulance_id;
		$(id).empty();
		var strUrl = Service.LOAD_VEHICLES;
...
function loadSubstore() {
	try {
		$('#substore_id').empty();
		var strUrl = Service.LOAD_SUBSTORE;
		console.log("LOAD_VEHICLES:::::: " + strUrl);
		var obj_Insert = {
			supStoreTypeId : 102
		};
...
function loadReturnItems(storeId, adjId, status_id, supplier_id) {
	...
	try {
		...
		var strUrl = Service.LOAD_RETURN_DRUGS;
...
var vendor_id = $('#vendor_id').val();
	...
	var strUrl = Service.SAVE_RETURN_DRUGS;
