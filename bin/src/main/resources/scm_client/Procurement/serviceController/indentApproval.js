...
function getAllIndentDetails() {
	try {
		var strUrl = Service.GET_ALL_INDENT_DETAILS;
...
function rejectIndent() {
	...
	if (rejection_remarks_id == "" || rejection_remarks_id == ''
		...
		showNotificationError("Enter Remarks", "rejection_remarks_id", "error");
		return;
	}
	var obj_Insert = {
	...
	var strUrl = Service.UPDATE_INDENT_REJECTION;
...
function getApprovalItemData(indent_id) {
	try {
		...
		var strUrl = Service.APPROVAL_ITEM_LIST;
...
function onclickCheckbox2(index) {
	...
	$("input:checkbox[name=case]:checked").each(function() {
		...
		quantity = $('#description_id'+index).val();
		$('#quanity').val(quantity);
	});
}
...
function getIndentData(indentId) {
	...
	var strUrl = Service.INDENT_DATA;
...
function indentApproval() {
	...
	var strUrl = Service.UPDATE_INDENT_DETAILS;
...
function rejectApproval() {
	...
	var strUrl = Service.REJECT_INDENT;
