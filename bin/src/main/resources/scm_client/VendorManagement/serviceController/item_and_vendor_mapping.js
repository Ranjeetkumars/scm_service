...
$("#vendor_name_id").change(function() {
	...
	alert('loadMappedDrugs java script function is executed::' + vendor_id);
	loadMappedDrugs();
});
...
function loadUnMappedDrugNames() {
	...
	var strUrl = Service.loadDrugNames;
	alert('url::' + strUrl);
	$.ajax({
		type : "POST",
		url : strUrl,
...
function loadMappedDrugs() {
	...
	var strUrl = Service.loadMappedDrugs;
	alert('url::' + strUrl);
	$.ajax({
		type : "POST",
		url : strUrl,
...
function loadVendorName() {
	alert('loadVendorName function is executed');
	try {
		$('#vendor_name_id').empty();
		var strUrl = Service.listloadSuppliers;
...
function mappedWithVendor() {
	...
	var strtDate =	$("#delevryDate").val();
	 var pgFormat = moment(strtDate).format('YYYY-MM-DD');
...
function validateFields() {
	...
	/*
	 * if (strtDate == null) { alert("Please enter start date"); return false; }
	 */
	/*
	 * if (parseInt(purchase_unit_price)>parseInt(purchasePrice)) {
	 * alert("Purchase unit price not more than purchase price"); return false; }
	 * if (parseInt(purchase_unit_price) >parseInt(mrp)) { alert( " Unit price
...
function appendIntoTable() {
	alert('appendIntoTable function is executed');
	...
	alert('equipment_DOM function is executed');
	// For Div Tag
	try {
