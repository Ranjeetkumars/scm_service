 ...
 $('#equipmentTableId').empty();
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(tablcol7)
					.append(
							'<a href="#">Map Vendor<i class="fa fa-trash"></i><i></a> ');
			$(tablcol7).attr('onclick', 'updateDrugSupplierMapping("' +
					  strData[i].serialId + '","' + strData[i].drugName + '","' +
					  strData[i].shortCode + '","' + strData[i].drugbrandLang +
					  '")');
...
function mappedDrugs(strData) {
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(tablcol7)
					.append(
							'<a href="#">Unmappe<i class="fa fa-trash"></i><i></a> ');
