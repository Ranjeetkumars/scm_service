...
function getAdjustmentType() {
	try {
		...
		var strUrl = Service.ADJUSTMENT_TYPE;
...
function getStores() {
	try {
		$('#store_id').empty();
		var strUrl = Service.LOAD_STORE_DROPDOWN;
...
function getBrands() {
	try {
		$('#brand_id').empty();
		var strUrl = Service.LOAD_BRABDS_DROPDOWN;
...
function getForms() {
	try {
		$('#form_id').empty();
		var strUrl = Service.LOAD_FORM_DROPDOWN;
...
function getCompanies() {
	try {
		...
		var strUrl = Service.LOAD_MANUFACTURE_COMPANIES_DROPDOWN;
...
function getAdjustableItems() {
	try {
		...
		var strUrl = Service.STOCK_ADJUSTMENT_SEARCH;
...
function get_RowData(drug_name,brand_name,unicode,form_type,strength_type,batch_number,available_stock,expire_date,mrp,unit_cost,invoice_number,drug_id, stock_ref){
	$('#viewTable_id').empty();
...
if(description_id === "0" || description_id === 0){
	...
	var strUrl = Service.SAVE_ADJUSTED_STOCK_DATA;
...
function updateStockQuantity() {
	...
	var strUrl = Service.UPDATE_STOCK_QUANTITY;
