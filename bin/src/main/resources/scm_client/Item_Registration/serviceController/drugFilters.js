...
function itemSearchByFilter() {
	...
	try {
		...
		var strUrl1 = "http://localhost:2000/scmservice/salesIndentRaisedController/getMedicines";
		var strUrl = strUrl1;
		alert("strUrl : " + strUrl);
		$.ajax({
			type : "POST",
...
function get_RowData(genericName, serialId, shortUnicCode, drugBrandLang_1,
		...
		dr_MinSalesQty, subStoreMinQty, subStoreMaxQty, barCode) {
	...
	$('#reg_drug_centralMaxQtyId').val(dr_MinmunLevelQty);
	...
	$('#reg_drreg_expiryalertId').val(dr_ExpireAlert);
