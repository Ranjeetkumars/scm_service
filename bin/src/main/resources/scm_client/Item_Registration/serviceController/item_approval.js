...
function itemApproval() {
	var strUrl = "http://localhost:2000/scmservice/itemApproval/getListOfActiveDrugs";
	var activeId = {
		"activeId" : "1"
	};
	$.ajax({
...
function appendInItemApprovalTable(jsonArray) {
	...
	for (var i = 0; i < jsonArray.length; i++) {
		...
		var dom = '<tr>' + '<td><label class="check">'
				...
				+ '<td>Bharat Medical Pvt Ltd</td>' + '<td>'
				+ jsonArray[i].drugBrandLang1 + '</td>' + '<td>ddd</td>'
				+ '</tr>'
...
function test(seriaalId) {
	...
	var strUrl = "http://localhost:2000/scmservice/itemApproval/getListOfDrugsDetailsBasedOnDrugId";
	var drugId = {
		"strdrugId" : seriaalId
	};
	$.ajax({
...
function itemRejection() {
	...
	var strUrl = "http://localhost:2000/scmservice/drugRegisteringService/updateActiveDrugForRejectApproval";
