	...
	try {
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						if (200 !== responsecode) {
							...
							$.each(jsonArray, function(i, resData) {
								...
								alert("resData.manufactureId"
										+ resData.manufactureId);
								alert("resData.companyName"
										+ resData.companyName);
					...
					error : function(err) {
						alert("Error in getManufactureDropDown"
								+ JSON.stringify(err));
					}
				});
	} catch (err) {
...
function getInventoryItemFormDropDown(inventoryGroupIdZoneId,
		reg_drug_inventoryItemFormId) {
	alert("getInventoryItemFormDropDown method is executed");
	try {
...
function getPackingDropDown(inventoryItemFormId, reg_drug_packingId) {
	alert('packing type method is executeed ');
	try {
...
function updateDrugsDetails() {
	alert('update function is executed');
	...
	if (globalIds.drugId > 0) {
		var strUrl = "http://localhost:2000/scmservice/drugRegisteringService/updateDrugDetails";
		alert('strUrlstrUrl::' + strUrl);
		var obj_Insert = {
		...
		$.ajax({
			...
			success : function(data) {
				console.log('@@@@@@@@@@@@@@@@@@@@@:------------data'
						+ JSON.stringify(data));
				// toastr.warning('Something went wrong!'+JSON.stringify(data));
				if (data.rtnReponseCount === 0 || data.rtnReponseCount === "0"
						|| data.rtnReponseCount === '0') {
					/*
					 * resetItemSearch(); toastr.info('Item Already Exist!');
...
function insertDrugDetails() {
	alert("insertDrugDetails method is executed");
	...
	alert('==== Obj_Insert' + JSON.stringify(JsonEmptyOrNot));
	...
	try {
		...
		$
				.ajax({
					...
					error : function(err) {
						alert("Error in getinventoryItemFormwithoutInputDropDown"
								+ JSON.stringify(err));
					}
				});
	} catch (err) {
...
function drugValidation() {
	alert("drugValidation function is executed");
	...
	if (!drugName.match(name_regex) || drugName.length == 0) {
		toastr.warning('Please enter valid Drug Name!'); // This Segment
		return false
	}
	 ...
	 */
	...
	else if (reg_drug_inventoryItemFormId == '0'
		...
		return false
	} else if (substoreMinQty == '' || substoreMinQty == null
		...
		toastr.warning('Please Enter Sub Store Min Qty!');
		return false
	} else if (subStoreMaxQty == '' || subStoreMaxQty == null
			|| subStoreMaxQty == 'undefined') {
		toastr.warning('Please Enter Sub Store Max Qty!');
		return false
	} else if (ambMinQty == '' || ambMinQty == null || ambMinQty == 'undefined') {
		toastr.warning('Please Enter Ambulance To Ambulance Min Qty!');
		return false
	} else if (ambMaxQty == '' || ambMaxQty == null || ambMaxQty == 'undefined') {
		toastr.warning('Please Enter Ambulance To Ambulance Min Qty!');
		return false
		...
		return false
	} else if (expAlert == '' || expAlert == null || expAlert == 'undefined') {
...
function DrugMiscellaneousRegistation() {
	...
	alert('concatSnoAndItemName::'+concatSnoAndItemName);
	...
	if(parseInt(centralMaxQtyId) < parseInt(centralMinQtyId)){
     ...
     if(genericNameId==null||genericNameId=='null'||genericNameId=="0"||genericNameId==0){
    	 genericNameId =1;
     }
