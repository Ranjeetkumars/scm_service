...
function loadList(strData) {
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(buttonTag).attr('onclick', 'getSingleRowData("'+i+'")');*/

	 		var tablcol11 = document.createElement("td");
			$(tablcol11).html('<button type="button" class="btn btn-primary btn-sm"   onClick="getSingleRowData('+i+','+strData[i].drr_drug_id+');" >Transfer Item</button>');
...
function getVehicleAlsBls(vehicleId) {
	...
	var rtnVehicleType;
	//var strUrl = "http://localhost:2000/scmservice/addNewDrugController/getVehicleAlsBls";
	...
	$.ajax({
		...
		crossDomain : true,
		success : function(data) {
         alert("inside vehicleAlsBls function::"+JSON.stringify(data));
...
function getVehicleDrugStatus(vehicleId,drugid) {
	...
	//var strUrl = "http://localhost:2000/scmservice/addNewDrugController/getVehicleDrugStatus";
